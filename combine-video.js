    ...
    $scope.combineSelectedVideo = () => {
      ...
      if (files.length > 0) {
        for (let file of files) {
          ...
          if (!file.match(/\.(mp4|mkv|avi|flv|jpg|png|ass|ssa|srt)$/)) continue;
          let nameWithoutEXT = file.replace(/(-\d{1,3})?\.(mp4|mkv|avi|flv|jpg|png|ass|ssa|srt)$/, ''),
            part = file.match(/-(\d{1,3})+\.(mp4|mkv|avi|flv|jpg|png|ass|ssa|srt)$/),
